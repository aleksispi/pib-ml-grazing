...
import numpy as np
import config
from utils import read_image_and_polygon, align_poly_with_image, create_square_bounding_box, mlp_inference
...
CSV_PATH = '../jv-data/csv_subset_all_2022.npy'  # Path to csv npy file with activity labels (only used for 2022 data)
PKL_SAVE_NAME = 'ml_dataset.pkl'  # Name of the pickle file to save the dataset to
DEVICE = 'cpu'
SEED = 0
...
NEXT_TIMESERIES_WHEN_CLOUD_FREE = False  # True --> Skip to next timeseries when no clouds are detected
...
START_DATE = '2022-04-01'  # Start date for the timeseries
END_DATE = '2022-10-21'  # End date for the timeseries (exclusive)
...
for file_idx in range(START_IDX, END_IDX):
    ...
    im_series_for_cot = np.copy(im_series)
    im_series_for_cot = im_series_for_cot[:, :, 1:, :]  # Skip band 1 for COT model, but keep also the original im_series for later saving.
    ...
    if config.IMG_ZOOMED_SIZE is not None:
        ...
        im_series_for_cot = im_series_for_cot[h_start : H//2+config.IMG_ZOOMED_SIZE//2, w_start : W//2+config.IMG_ZOOMED_SIZE//2, :, :]

    # Recompute some things based on the new image size
    H, W, _, _ = im_series_for_cot.shape
    ...
    # Go over which time steps to keep
    t_to_keep = []
    pred_maps_binary = []
    for t in range(T):
        # Perform prediction
		...
		if False:
        	img = (im_series_for_cot[:, :, :, t] - 1000) / 10000  # -1k and then 10k division, done after Jan 2022
		else:
			img = im_series_for_cot[:, :, :, t] / 10000
        pred_map, pred_map_binary_list, _ = mlp_inference(img, means, stds, models, H*W, config.THRESHOLD_THICKNESS_IS_CLOUD,
        ...
        if not (DISCARD_CLOUDY_POLYS and pred_cloudy_poly):
            t_to_keep.append(t)
            pred_maps_binary.append(pred_map_binary[:, :, np.newaxis])

    # Append im_series and label to the dataset, as well as dates and the polygon mask and xy-coordinate-poly.
    im_series_to_keep = im_series[:, :, :, t_to_keep]
    pred_maps_binary = np.concatenate(pred_maps_binary, axis=2)
    dates_to_keep = [dates[i] for i in t_to_keep]
    ml_dataset.append((im_series_to_keep, label, dates_to_keep, poly_mask, poly_aligned_int, pred_maps_binary, date_stamp, poly))
...
for _, label, _, _, _, _, _, _ in ml_dataset:
    if label != -1:
...
idx = 0
idxs_used = []
for im_series, label, dates, poly_mask, poly_aligned_int, pred_map_binary, date_stamp, poly in ml_dataset:
    if idx in idxs_used:
        idx += 1
        continue
    if label == 0:
        ...
        if label_counts[0] <= nbr_val:
            val_dataset.append((im_series, label, dates, poly_mask, poly_aligned_int, pred_map_binary, date_stamp, poly))
            ...
            for close_idx in close_idxs:
                if close_idx not in idxs_used:
                    im_series, label, dates, poly_mask, poly_aligned_int, pred_map_binary, date_stamp, poly = ml_dataset[close_idx]
                    val_dataset.append((im_series, label, dates, poly_mask, poly_aligned_int, pred_map_binary, date_stamp, poly))
            ...
            train_dataset.append((im_series, label, dates, poly_mask, poly_aligned_int, pred_map_binary, date_stamp, poly))
            idxs_used.append(idx)
